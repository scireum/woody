...
public class CompanyController_old extends BizController {
    ...
    @Routed("/companies")
    @LoginRequired
    @Permission(CompanyController_old.PERMISSION_MANAGE_XRM)
    public void companies(WebContext ctx) {
        PageHelper<Company> ph =
                PageHelper.withQuery(oma.select(Company.class).orderAsc(Company.CUSTOMERNR).orderAsc(Company.NAME))
                          .forCurrentTenant();
        ph.withContext(ctx);
