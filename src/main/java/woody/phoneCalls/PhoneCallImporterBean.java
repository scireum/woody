...
* id, auch cdrId		long		Fortlaufende Listen-ID
 ...
 ankommenden und abgehenden Rufen zu unterscheiden
	...
	private static final String INITIALIZE_NORMALIZED_PHONE_NUMBERS_IN_PERSON = "InitializeNormalizedPhoneNumbersInPerson";
...
//		}
		...
		try {
			...
			System.err.println(NLS.toUserString(LocalDateTime.now()) + " PhoneCallImporter =OCMDatasource: " + db.toString() + " ======= sql: " + sql);
			List<Row> rowList = db.createQuery(sql).queryList();
			System.err.println(NLS.toUserString(LocalDateTime.now())+ " PhoneCallImporter =LIST, size " + rowList.size() + " ======= " + NLS.toUserString(LocalDateTime.now()));
			numFetched = importPhoneCalls (rowList);
			LocalDateTime endDate = LocalDateTime.now();
			long diffTime = ChronoUnit.SECONDS.between(startDate, endDate);

			System.err.println(NLS.toUserString(LocalDateTime.now()) + " PhoneCallImporter =FINISH=== " + NLS.toUserString(numFetched) +
							   " RECORDS ===== " + NLS.toUserString(endDate) +
							   " === " + NLS.toUserString(diffTime) + "s, lastPhoneCall: " + lastPhoneCallString);
		} catch (Throwable t) {
	 ...
	 * Zusätzlich wird aus allen CDR ein best mögliches Ergebnis in phoneCall gespeichert und das top-Flag zur Selektion gesetzt.
     */
	private void importGroup(List<Row> rowList, int start, int end) {
		...
		for(int i = start; i<end; i++) {
			...
			if ((oma.select(PhoneCall.class)
					.eq(PhoneCall.CDRID, cdrId).exists())) {
				rowExists = true;
			} else {
				...
				if(save) {
					if (!(oma.select(PhoneCall.class)
							.eq(PhoneCall.CDRID, phInterims.getCdrId()).exists()))
		...
		save = save1 && save2;                            		phoneCall.setSave(save);

		// store the phoneCall
		if (!(oma.select(PhoneCall.class)
				.eq(PhoneCall.CDRID, phoneCall.getCdrId())
				.eq(PhoneCall.TOP, true).exists())) {
			oma.update(phoneCall);
			// ToDO Rechtsnachfolgen für Syslog
...
//			Syslog.log("ERROR PHONECALL-IMPORT", "NOT Imported: " + phoneCall);
     ...
     */
	// ToDO : was (wer) ist Rechtsnachfolger für row.getFields()
	private String identifyDifference(Row row, Row lastRow) {
		Map<String, Object> rowMap = row.getFields();
		Map<String, Object> lastRowMap = lastRow.getFields();
		String text = "";
		for(String key: rowMap.keySet()) {
			if(key.contains("time"))  { continue; }
			if("callleguuid".equals(key))  { continue; }
			if("id".equals(key))  { continue; }
			Object value = rowMap.get(key);
			Object other = lastRowMap.get(key);
			...
			String ot = "null";
			String vt = "null";
			if(value != null) {vt = value.toString();}
			if(other != null) {ot = other.toString();}
	...
	private boolean disablePhoneCall(PhoneCall phoneCall, boolean transfer) {
		...
		if(Strings.isFilled(phoneCall.getCalledcallerid()) && "unknown".equals(phoneCall.getCalledcallerid().toLowerCase()))  {
...
//						.eq(lastname, Employee.LASTNAME).first();
				...
				uac = oma.select(UserAccount.class)
							  ...
							  .eq(UserAccount.PERSON.inner(PersonData.LASTNAME), lastname).queryFirst();
...
//						.eq(inaktiv, Employee.INAKTIV).first();
				uac = oma.select(UserAccount.class)
							  ...
							  .eq(Employee.INAKTIV, inaktiv).queryFirst();
...
//										 .icontains(phonenumber, Employee.PBXID).list() ;
		...
		List<UserAccount> employeeList = oma.select(UserAccount.class).eq(Column.named("Employee_" + Employee.PBXID), phonenumber).queryList();
//		List<Employee> employeeList = oma.select(Employee.class).eq(Employee.PBXID, phonenumber).queryList();
	...
	private void lookPerson(PhoneCall phoneCall, String phoneNumber) {
		...
		if(phoneNumber.length() > 5) {
			...
			Optional opti  = oma.select(Person.class)
						.eq(Person.CONTACT.inner(ContactData.PHONE), phoneNumber).first();
			if(opti.isPresent()) {
				person = (Person) opti.get();
			} else {
				 opti  = oma.select(Person.class)
									.eq(Person.CONTACT.inner(ContactData.MOBILE), phoneNumber).first();
				if(opti.isPresent()) {
					person = (Person) opti.get();
				}
	...
	public void execute(Output output, String... strings) throws Exception {
		// das wird beim Minutenwechsel nicht aktiviert
	}
