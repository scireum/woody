...
import woody.sales.contracts.CompanyAccountingData;
...
public class OfferItem extends BizEntity {
    ...
    public List<PackageDefinition> getAllPackageDefinitionsOrderedByProduct() {
        List<PackageDefinition> pdList = oma.select(PackageDefinition.class)
                                            .eq(PackageDefinition.PRODUCT.join(Product.TENANT),
    ...
    protected void onSave() {
            ...
            if (isSum()) {
                ...
                if (Strings.isEmpty(this.getKeyword())) {
                    throw Exceptions.createHandled()
                                    .withNLSKey("OfferItem.keywordMissing")
                                    .set("pos", position)
                                    .handle();
                }
                ...
                if (isService()) {
                    ...
                    if (singlePrice == null || singlePrice.isZeroOrNull()) {
                        ...
                        CompanyAccountingData companyAccountingData = null;//company.getCompanyAccountingData();
                ...
                if (Strings.isEmpty(quantityUnit)) {
                    if (ProductType.LICENSE.equals(packageDefinition.getValue()
                                                                    .getProduct()
                                                                    .getValue()
                                                                    .getProductType())) {
                        quantityUnit = packageDefinition.getValue().getAccountingUnit();
                    }
                    if (ProductType.SERVICE.equals(packageDefinition.getValue()
                                                                    .getProduct()
                                                                    .getValue()
                                                                    .getProductType())) {
                        quantityUnit = "PT";
        ...
        if (flagOneTime) {
            ...
            if (position == null) {
                OfferItem offerItem = oma.select(OfferItem.class)
                                         .eq(OfferItem.OFFER, offer)
                                         .orderDesc(OfferItem.POSITION)
                                         .queryFirst();
            ...
            if (!(isSum() || isInfoText())) {
                ...
                if (Strings.isFilled(history)) {
                    ...
                    if (md5SubStr.equals(md5)) {
                        return;
                    }   // md5-key is the same --> no change done
                }
                String stateFix = getStateFix(14);
                String username = userInfo.getUserName();
                String s = NLS.toUserString(LocalDateTime.now())
                           + "   "
                           + username
                           + ":  "
                           + getStateFix(14)
                           + ", md5: "
                           + buildMD5(OfferItem.this);
                if (history == null) {
                    history = "";
                }
    ...
    private String buildMD5(OfferItem o) {
        ...
        if (o.isInfoText() || o.isSum()) {
            ...
            s = s
                + o.getPriceBase()
                + NLS.toUserString(o.getSinglePrice())
                + NLS.toUserString(o.getQuantity())
                + o.getQuantityUnit()
...
//    }
    ...
    public boolean isPackageDefinitionSinglePricePresent() {
        if (packageDefinition.getValue().getSinglePrice() != null && packageDefinition.getValue()
                                                                                      .getSinglePrice()
                                                                                      .isPositive()) {
            return true;
        }
    ...
    public boolean showNextState() {
        if (OfferItemState.UNUSED.equals(this.getState())) {
            return false;
        }
        if (OfferItemState.CANCELED.equals(this.getState())) {
            return false;
        }
        if (OfferItemState.COPY.equals(this.getState())) {
            return false;
        }
        return true;
    }
