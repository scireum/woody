                    ...
                    if (!contractList.isEmpty()) {
                        ...
                        if (toDoList.size() > 0) {
                            ...
                            for (ContractToDos<Contract, Contract, Contract, Integer> c : toDoList) {
                                ContractToDos.Command command = c.getCommand();
                    ...
                    if (contractList.size() > 0) {
                        for (Contract contract : contractList) {
                            ...
                            if (PackageDefinition.ACCOUNTINGPROCEDURE_VOLUME.equals(accountingProcedure)) {
                                ContractToDos.Command command = analyzeVolumeContract(contract, referenceDate);
                                if (companyInvoiceNr != invoiceNr) {
                                    invoiceNr--;
                                    companyInvoiceNr = invoiceNr;
                                }
...
//              .loginRequired(true).setUser(Users.getCurrentUser()).publish();
     ...
     * contract
     */
    private ContractToDos.Command analyzeVolumeContract(Contract contract, LocalDate referenceDate) {
        if (contract.getStartDate().isAfter(referenceDate)) {
            ...
            return ContractToDos.Command.NOTHING;
        }
        if (contract.getStartDate().equals(contract.getEndDate())) {
            return ContractToDos.Command.FINISHED_CONTRACT;
        }
        if (contract.getAccountedTo() == null) {
            return ContractToDos.Command.NEW_CONTRACT;
        } else {
            if (contract.getEndDate() != null) {
                ...
                if (contract.getAccountedTo().isBefore(contract.getEndDate())) {
                    return ContractToDos.Command.ACC_IS_BEFORE_TO;
                } else {
                    return ContractToDos.Command.ACC_IS_AFTER_TO;
                }
            }
            return ContractToDos.Command.ACC_IS_PRESSENT_TO_IS_NULL;
        }
    }
    ...
    private void writeLineitem(DataCollector<Lineitem> itemCollector,
        ...
        String outputLanguage = company.as(CompanyAccountingData.class).getOutputLanguage();
        ...
        String customerNr = contract.getCompany().getValue().getCustomerNumber();
        ...
        switch (command) {
            ...
            case ACC_IS_BEFORE_TO:  // case 1, F5
                if (c1.getAccountedTo()
                    ...
                    nextAccTo = getNextAccountedTo(c1, c2, referenceDate);
                    c1 = accountContract(itemCollector, dryRun, c1, c1.getAccountedTo(), nextAccTo, INVOICE,
                                         // 17.2.2016
                                         referenceDate, AccountingService.RUNNINGACCOUNTING, invoiceNr, clearingDate);
                    save = true;
                }
                break;
    ...
    private List<ContractToDos<Contract, Contract, Contract, Integer>> checkContractList(List<Contract> contractList,
        ...
        for (Contract c : contractList) {
            ContractToDos.Command command = analyzeContract(c, referenceDate);
     ...
     */
    private ContractToDos.Command analyzeContract(Contract c, LocalDate referenceDate) {
        if (c == null) {
            return ContractToDos.Command.NOTHING;
        }
        ...
        if (c.getAccountedTo() != null) {
            if (c.getEndDate() != null) {
                if (c.getAccountedTo().equals(c.getEndDate())) {
                    return ContractToDos.Command.FINISHED_CONTRACT; // finished contract
                }
                ...
                if (c.getAccountedTo().isBefore(c.getEndDate())) {
                    command = ContractToDos.Command.ACC_IS_BEFORE_TO; // acc < to --> INVOICE
                } else {
                    command = ContractToDos.Command.ACC_IS_AFTER_TO; // acc > to -->CREDIT
                }
            } else {
                ...
                command = ContractToDos.Command.ACC_IS_PRESSENT_TO_IS_NULL; // acc is present, to is null
            }
        } else {
            if (c.getEndDate() == null) {
                command = ContractToDos.Command.NEW_CONTRACT; // new contract
            } else {
                command = ContractToDos.Command.TO_IS_PRESENT_ACC_IS_NULL; // to is present, acc = null
            }
        }
        return command;
    }
...
//                  .publish();
        ...
        for (Lineitem l : lineitemList) {
            if (filterFlag) { // filter is != nein
                ...
                Company company = oma.select(Company.class).eq(Company.CUSTOMER_NUMBER, customerNr).queryFirst();
...
//                    System.err.println(text);
     ...
     * @return
     */
    private Lineitem checkItem(Lineitem item, boolean collmexIsCredit, String status, Amount finalDiscountSum) {
        boolean flag = false;
...
//            return new File(Path.getPath(LINEITEM_PATH).getFile(), s + name + ".csv");
    ...
    private Amount generateCollmexInvoiceLine(PrintWriter pw, Lineitem lineitem, LocalDate invoiceDate) {
        ...
        csv[40] = "Position am " + NLS.toUserString(LocalDateTime.now()) + " exportiert"; // 40 Internes Memo C 1024
...
//        }
    ...
    public void generateCollmexKundenCsv() {
        List<Company> companyList = oma.select(Company.class)
                                       .where(FieldOperator.on(Company.CUSTOMER_NUMBER).notEqual(""))
                                       .orderAsc(Company.CUSTOMER_NUMBER)
                                       .queryList();
        ...
        for (Company c : companyList) {
            System.err.println(c.toString() + "/" + c.getCustomerNumber() + "/");
        }
        if (companyList.size() > 0) {
            return;
        }
...
//                      .publish();
    ...
    private void printKundenCsvLine(PrintWriter pw, Company company) {
        StringBuilder sb = new StringBuilder();
        sb.append("CMXKND;");
        sb.append(company.getCustomerNumber());
